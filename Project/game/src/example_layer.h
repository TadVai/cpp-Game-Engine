#pragma once
#include <engine.h>
#include "player.h"
#include "cross_fade.h"
#include "enemy.h"
#include "billboard.h"

class example_layer : public engine::layer
{
public:
	example_layer();
	~example_layer();

	void on_update(const engine::timestep& time_step) override;
	void on_render() override;
	void on_event(engine::event& event) override;
	int get_lives();

private:
	void check_bounce();
	void jumping_sound();
	void walking_sound();

	engine::ref<engine::skybox>			m_skybox{};
	engine::ref<engine::game_object>	m_terrain{};
	engine::ref<engine::game_object>	m_cow{};
	engine::ref<engine::game_object>	m_tree{};
	engine::ref<engine::game_object>	m_mannequin{};
	engine::ref<engine::game_object>	m_spaceship{};
	engine::ref<engine::game_object>	m_truck{};
	engine::ref<engine::game_object>	m_logs{};
	engine::ref<engine::game_object>	m_wolf{};
	engine::ref<engine::game_object>	m_well{};
	engine::ref<engine::game_object>	m_rock{};
	player								m_player{};
	enemy								m_enemy{};

	engine::ref<engine::material>		m_material{};

	engine::DirectionalLight            m_directionalLight;
	engine::PointLight					m_pointLight;
	uint32_t							num_point_lights = 1;
	engine::SpotLight					m_spotLight;
	uint32_t							num_spot_lights = 1;

	std::vector<engine::ref<engine::game_object>>     m_game_objects{};

	engine::ref<engine::bullet_manager> m_physics_manager{};
	engine::ref<engine::audio_manager>  m_audio_manager{};
	float								m_prev_sphere_y_vel = 0.f;
	engine::ref<engine::text_manager>	m_text_manager{};

	engine::orthographic_camera       m_2d_camera;
	engine::perspective_camera        m_3d_camera;

	engine::ref<cross_fade>			  m_cross_fade{};
	engine::ref<billboard>			  m_billboard{};

	bool m_fog = false;
	glm::vec3 m_fog_colour;
	float m_fog_factor_type;

	int m_score = 0;
	int m_lives = 5;
	int m_time = 0;
};
