...
{ 
    ...
    // inverting the transform matrix  
    //m_view_mat = glm::inverse(transform); 
    m_view_mat = glm::lookAt(m_position, m_front_vector+m_position , m_up_vector);
    m_view_projection_mat = m_projection_mat * m_view_mat; 
}
...
void engine::perspective_camera::update_camera_vectors()
{
	...
	//added glm::length
	front.x = m_position.x +(cos(pitch_radians) * glm::radians(glm::length(m_front_vector)));
    front.y = m_position.y - glm::radians(glm::length(m_up_vector));
    front.z = m_position.z + (sin(pitch_radians) * glm::radians(glm::length(m_front_vector)));
